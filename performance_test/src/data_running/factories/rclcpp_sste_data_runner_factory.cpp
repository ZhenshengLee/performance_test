...
#include "rclcpp_sste_data_runner_factory.hpp"
      ...
      if (T::msg_name() == msg_name) {
        switch (run_type) {
          case RunType::PUBLISHER:
            ptr = std::make_shared<
              DataPublisher<RclcppStaticSingleThreadedExecutorCommunicator<T>>>(
              stats);
            break;
          case RunType::SUBSCRIBER:
            ptr = std::make_shared<DataSubscriber<
              RclcppStaticSingleThreadedExecutorCommunicator<T>>>(stats);
            break;
        }
      }
    });
